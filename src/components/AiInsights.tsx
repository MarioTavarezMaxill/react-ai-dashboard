...
const impactConfig: Record<AiInsight['impact'], { color: string; label: string }> = {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 h-full">
      <div className="flex items-center gap-2 mb-5">
        ...
        <span className="ml-auto text-xs bg-violet-600/20 text-violet-400 px-2 py-0.5 rounded-full border border-violet-500/30">
          Live
        </span>
      </div>
      <div className="space-y-3">
        {aiInsights.map((insight) => {
          ...
          return (
            <div
              key={insight.id}
              className="p-4 bg-gray-800/50 border border-gray-700/50 rounded-xl hover:border-violet-500/30 transition-colors cursor-pointer"
            >
              <div className="flex items-start gap-3">
                <div className="p-2 bg-gray-700 rounded-lg flex-shrink-0">
                  <CategoryIcon size={14} className="text-gray-300" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className="text-sm font-medium text-white">{insight.title}</span>
                    <span className={clsx('text-xs px-1.5 py-0.5 rounded font-medium', impact.color)}>
                      {impact.label}
                    </span>
                  </div>
